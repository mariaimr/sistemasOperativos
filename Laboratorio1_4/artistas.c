...
typedef struct {
    char realName[MAX_CHARS]; //nombre real
    char nickname[MAX_CHARS]; //apodo
    char masterpiece[MAX_CHARS]; //obra mas representativa
    int artworks; //numero total de horas
} artist;

/* Declaration of functions (prototypes). */
void getOption(char *_option); //obtener opcion
void registerArtists(artist *art, int *n, int max); //Registrar
void showArtists(artist *art, int n); //mostrar artistas
